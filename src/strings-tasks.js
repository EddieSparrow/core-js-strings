...
function removeFirstOccurrences(str, value) {
  ...
  const indexNewStart = str.substring(0, indexOfValue);
  const indexNewEnd = str.substring(indexOfValue + value.length, str.length);
  return indexNewStart + indexNewEnd;
}
 ...
 * @param {string} value - The substring to remove from the string.
 * @return {string} - Th
 * e string with the last occurrence of the substring removed.
 *
 * @example
...
function formatTime(minutes, seconds) {
  ...
  const b = ':';
  const c = String(seconds).padStart(2, '0');
  return a + b + c;
}
...
function countVowels(str) {
  const vovelsArray = 'aeiouyAEIOUY'.split('');
  const strArr = str.split('');
  let sumOfVovels = 0;
  for (let i = 0; i < vovelsArray.length; i += 1) {
    if (vovelsArray.includes(strArr[i])) {
      sumOfVovels += 1;
    }
  }
  return sumOfVovels;
...
function isPalindrome(str) {
  const testStr = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
...
function findLongestWord(sentence) {
  const sortWord = sentence.split(' ').sort((a, b) => b.length - a.length);
  return sortWord[0];
}
...
function extractNameFromTemplate(value) {
  const strToFind = 'Hello, ';
  const strStart = value.indexOf(strToFind) + strToFind.length;
  return value.substring(strStart, value.length - 1);
}
...
function encodeToRot13(str) {
  ...
  const sa = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  const strToArr = str.split('');
  let rotStr = '';
  for (let i = 0; i < strToArr.length; i += 1) {
    const index = alphabet.indexOf(strToArr[i]);
    if (index === -1) {
      rotStr += strToArr[i];
    }
    rotStr += sa.charAt(index);
  }
  return rotStr;
}
